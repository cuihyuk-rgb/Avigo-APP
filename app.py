...
def load_configs():
    ...
    return cal, feat_cfg, stats, coords, airports_db, categories, category_types
cal, feat_cfg, stats, coords, airports_db, categories, category_types = load_configs()
# =========================== 模型按需加载 ===========================
...
def predict_price(df_in):
    ...
    for name in ['P05', 'P50', 'P95']:
        m = load_single_model(name)
        results[name] = float(m.predict(df_in)[0])
        del m
        gc.collect()
    p05 = results['P05']
    p50 = results['P50']
...
if mode == '单次精准报价':
    ...
    if submitted:
        ...
        if errors:
            ...
            with st.spinner('正在计算价格（首次需加载模型，约1-2分钟）...'):
            ...
            with c2:
                ...
                with st.popover('📋 查看价格详情', use_container_width=True):
                    ...
                    st.caption(f'市场基准价约为 **${base_value:,.0f}**')
                    for item in contributions[:10]:
                        ...
                        st.markdown(
                            ...
                            f"{feat} "
                            f"<small style='color:gray;'>({val})</small>",
                            unsafe_allow_html=True
                        )
            c3.metric('📈 价格上限', f'${upper:,.0f}')
            st.caption(f'🛫 航线: {route} | 📏 距离: {dist_nm:,.0f} 海里 | ⏱️ 飞行时间: ~{flt_min:,.0f} 分钟')
                ...
                if not candidate_types:
                    ...
                    for cat, t in candidate_types:
                        df_in, _, flt_min, route, _ = build_features(
                            from_icao, to_icao, cat, t, 'Unknown', None, None, {}
                        )
                        lower, median, upper = predict_price(df_in)
                        results.append({
                            '飞机分类': cat,
